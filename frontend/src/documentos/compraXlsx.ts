...
export async function construirCompraXlsx(d: CompraData, direccion?: string): Promise<Blob> {
  ...
  setRows(ws, [
    ...
    15, 15, 15, 15, 15.8, 15.8, // 11-16: CEL/MAIL/CONDICION/IMEI/GARANTIA + aire
    ...Array(29).fill(15), // 17-45
    15.8, 15, 25.5, 20.2, // 46-49
  ])
  ;['A1:J1', 'B6:I6', 'B7:I7', 'B8:I8', 'B9:I9', 'F11:I11', 'F12:I15', 'B17:I46'].forEach((m) => ws.mergeCells(m))
  ...
  b.caja(2, 11, 4, 15, [11, 12, 13, 14]) // CEL / MAIL / CONDICION / IMEI / GARANTIA
  b.apply(ws)
  cajaCompletaEn(ws, 'A1')
  cajaCompletaEn(ws, 'F11') // etiqueta TOTAL (combinada)
  cajaCompletaEn(ws, 'F12') // importe TOTAL (combinada F12:I15)
  cajaCompletaEn(ws, 'B17') // garantía (combinada)
  ...
  put(ws, 'B15', con(COMPRA_LABELS.garantia, d.garantia, COMPRA_LABELS.garantia), bold10, ALIGN.left)
  put(ws, 'F11', COMPRA_LABELS.total, calibri(11, true), ALIGN.center)
  put(ws, 'F12', d.total, calibri(16, true), ALIGN.center)

  ws.getCell('B17').value = richGarantia(COMPRA_GARANTIA, 7)
  ws.getCell('B17').alignment = { horizontal: 'left', vertical: 'top', wrapText: true }
