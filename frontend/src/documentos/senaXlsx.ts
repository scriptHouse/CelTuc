...
export async function construirSenaXlsx(d: SenaData): Promise<Blob> {
  ...
  put(ws, 'F1', d.numeroRecibo.trim() ? `${SENA.numeroRecibo} ${d.numeroRecibo}` : SENA.numeroRecibo, s10, ALIGN.center)
  put(ws, 'F2', SENA.fecha, calibri(10, true), ALIGN.center)
  put(ws, 'F3', d.fecha, s10, ALIGN.center)
